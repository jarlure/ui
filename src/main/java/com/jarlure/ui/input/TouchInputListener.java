...
public interface TouchInputListener {
     ...
     */
    void onPointPressShortTime(PointTouchEvent finger);

    /**
     * 长时间按压事件。该事件在onPointPress()触发后又持续按压0.5s后触发
     * @param finger    触点ID为0的触点事件。（换句话说，其他触点不会触发这个事件）
     */
    void onPointPressLongTime(PointTouchEvent finger);

    /**
     * 双触点按压事件。当ID为0和ID为1的两个点，其中一点处于按压状态而另一点按下时，触发该事件
     * @param twoFingers    触点ID为0和触点ID为1的触点构成的事件
     */
    void onLinePress(LineTouchEvent twoFingers);

    /**
     * 双触点拖拽事件。当ID为0和ID为1的两个点中，其中一点发生移动时，触发该事件。该事件常用于缩放图片
     * @param twoFingers    触点ID为0和触点ID为1的触点构成的事件
     */
    void onLineDragging(LineTouchEvent twoFingers);

    /**
     * 双触点释放事件。当ID为0和ID为1的两个点中，其中一点被释放时，触发该事件
     * @param twoFingers    触点ID为0和触点ID为1的触点构成的事件
     */
    void onLineRelease(LineTouchEvent twoFingers);
